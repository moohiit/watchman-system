            ...
            <tbody>
              ...
              if ($fromDate && $toDate) {
                if ($role == 'admin' && $selectedDepartment != 'select') {
                  $sql = "SELECT * FROM inqury_data WHERE `date` BETWEEN '$fromDate' AND '$toDate' AND status='Early' AND dprt = '$selectedDepartment'";
                } else if ($role == 'admin') {
                  $sql = "SELECT * FROM inqury_data WHERE `date` BETWEEN '$fromDate' AND '$toDate' AND status='Early'";
                } else {
                ...
                if ($role == 'admin') {
                  $sql = "SELECT * FROM inqury_data WHERE status='Early' AND dprt = '$selectedDepartment'";
                }
              }else {
                if ($role == 'admin') {
                  $sql = "SELECT * FROM inqury_data WHERE `date` = CURRENT_DATE AND status='Early'";
                } else {
                  $sql = "SELECT * FROM inqury_data WHERE `date` = CURRENT_DATE AND status='Early' AND dprt = '$department'";
                }
              }
              $result = mysqli_query($conn, $sql);
              $i = 1;
