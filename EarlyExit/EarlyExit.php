            ...
            <tbody>
              ...
              if ($fromDate && $toDate) {
                if ($role == 'admin' && $selectedDepartment != 'select') {
                  $sql = "SELECT * FROM inqury_data WHERE `date` BETWEEN '$fromDate' AND '$toDate' AND status='Early' AND dprt = '$selectedDepartment' ORDER BY date DESC";
                } else if ($role == 'admin') {
                  $sql = "SELECT * FROM inqury_data WHERE `date` BETWEEN '$fromDate' AND '$toDate' AND status='Early' ORDER BY date DESC";
                } else {
                  $sql = "SELECT * FROM inqury_data WHERE `date` BETWEEN '$fromDate' AND '$toDate' AND status='Early' AND dprt = '$department' ORDER BY date DESC";
                }
              } else if (!$fromDate && !$toDate && $selectedDepartment != 'select') {
                if ($role == 'admin') {
                  $sql = "SELECT * FROM inqury_data WHERE status='Early' AND dprt = '$selectedDepartment' ORDER BY date DESC";
                }
              }else {
                if ($role == 'admin') {
                  $sql = "SELECT * FROM inqury_data WHERE `date` = CURRENT_DATE AND status='Early' ORDER BY date DESC";
                } else {
                  $sql = "SELECT * FROM inqury_data WHERE `date` = CURRENT_DATE AND status='Early' AND dprt = '$department' ORDER BY date DESC";
                }
              }
              $result = mysqli_query($conn, $sql);
              $i = 1;
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                <tr>
                  ...
                  <td><?php echo $row["dprt"] ?></td>
                  <td><?php echo $row["contact"] ?></td>
                  <td><?php echo $row["reason"] ?></td>
                  <td><?php echo $row["authorisedBy"] ?></td>
